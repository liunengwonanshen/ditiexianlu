...
public class DateBuilder {
	...
	public DateBuilder() {};
	public DateBuilder(String pathname) throws IOException{
        ...
        for(int i=0;i<linenum;i++) {  //循环往lineSet中添加line
        	...
        	for(int j=1;j<linearr.length;j++) {  //循环往line中添加station
        		...
        		for(List<Station> l:lineSet) {  //处理换乘站
        			for(int k=0;k<l.size();k++) {
        				if(l.get(k).getName().equals(linearr[j])) {  
        					...
        					l.get(k).setLine(newline);//Station更新换乘线路
        					line.add(l.get(k));//添加站点
        					flag=1;
        					break;
        				}
        			}
        		...
        		if(flag==0)
        			line.add(new Station(linearr[j],linename));//站点名和所在的地铁线添加
        	}
